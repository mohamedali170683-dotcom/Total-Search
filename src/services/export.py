...
class DemandReportPDF:
    ...
    def generate(
        self,
        analysis_data: dict,
        title: str = "Search Distribution Analysis",
    ) -> bytes:
        """
        ...
        <br/><br/>
        Total Search Volume: <b>{analysis_data.get('total_demand', 0):,}</b>
        <br/>
        ...
        # Pie Chart
        if platform_data:
            elements.append(Paragraph("Search Distribution", self.styles["SectionTitle"]))
        ...
        elements.append(Paragraph(
            "Report generated by Total Search - Cross-Platform Search Intelligence",
            self.styles["Normal"]
        ))
...
class DemandReportPPTX:
    ...
    def _add_summary_slide(self, prs: "Presentation", data: dict):
        ...
        # Key metrics
        metrics = [
            ("Total Search", f"{data.get('total_demand', 0):,}"),
...
def generate_demand_report_pdf(analysis_data: dict, title: str = "Search Distribution Analysis") -> bytes:
    """Generate a PDF demand report."""
    generator = DemandReportPDF()
    return generator.generate(analysis_data, title)


def generate_demand_report_pptx(analysis_data: dict, title: str = "Search Distribution Analysis") -> bytes:
